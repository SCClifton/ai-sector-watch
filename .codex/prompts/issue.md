...
This command is tool-agnostic. The default tool name is `codex` because this file lives at `.codex/prompts/issue.md`. The mirror file `.claude/commands/issue.md` defaults to `claude-code`. If you are a different agent and reading this body directly, substitute `<TOOL>` with your own short identifier (e.g. `human`, `gemini`).
...
If the narration explicitly names a tool to use (e.g. "use claude-code", "as human", "tool: gemini"), record that as `<TOOL>` for Step 3. Otherwise `<TOOL>` defaults to `codex` from this file.
